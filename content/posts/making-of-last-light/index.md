...
To be honest I'm not a big fan of the native code editor of Pico-8, as it's hard for my eyes to parse the font used there. It's not friendly for code reviews either. So the solution for me was to create external Lua files and include them in Pico-8. That let me use Visual Studio Code and handle source control better.

## Sprite editing

Making pixel art is tricky, but it's less so when you draw with lower resolutions. Pico-8 editor uses 8x8 based sprites, so I could focus on more essential details when designing my assets. I also love the limited color pallete, it's perfect for minimal designs. 

## Sound editing

I have no knowledge in music theory, so this was the trickiest part. For the sound effects, it was basically just trials and errors with how I want the effects to be. But music was a different story. This was the part that humbled my creativity the most. I ended up finding royalty-free music in the community to get it over with. For the next games, it would be nice to collaborate with someone who can write music to create original soundtracks.
...
This needs to be stressed twice. The solution to the level map was not the end of the struggle. I was so used to developing with powerful platforms that I've taken for granted the available resources. With Pico-8, the more logic added the more tokens your game consumes, and 8192 token limit is quite challenging. 
...
Pico-8 is a fun experience for learning to make games with small scopes. I'd love to try with a more serious game engine like Unity soon, but it requires so much more time and effort. For now, I'm happy with what I learned, and I'm inspired to make more games in the near future.
